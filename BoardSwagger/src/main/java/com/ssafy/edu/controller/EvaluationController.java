...
//		}
//		if(!hostService.validateTeamIdWithHost(teamId, boardId, sponserEmail)) {
...
//	
//	//해당 보드에 대한 평가들 전부 표시
//	@ApiOperation(value="주최가가 해당 팀평가를 수정.", notes="여기서 team_id, board_id는 불필요하다. 리턴 값으로 succ, fail을 출력한다.")
...
//			@ApiParam(value = "수정하고자 하는 id", required = true) @PathVariable Integer teamEvaluationId, @ApiParam(value = "평가하고자 하는 team_id, board_id", required = true) @RequestBody UpdateEvaluateTeamRequest request){
...
//		}	
//		Team team = teamRepo.findByTeamId(te.getTeamId());
//		if(!hostService.validateTeamIdWithHost(team.getTeamId(), te.get, sponserEmail)) {
//			return CommonResponse.makeResponseEntity(-1, "TeamId, BoardId가 유효하지 않습니다.", CommonResponse.FAIL, HttpStatus.BAD_REQUEST);
//		}	
//		//여기까지 유효한지 아닌지 검증
//		
